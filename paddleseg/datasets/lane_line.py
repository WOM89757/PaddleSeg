...
class Lane_Line(Dataset):
    ...
    def __init__(self, transforms, dataset_root=None, mode='train', edge=False):
        ...
        if mode not in ['train', 'val']:
            raise ValueError(
                "`mode` should be one of ('train', 'val') in Lane_Line dataset, but got {}."
                .format(mode))
        ...
        if mode == 'train':
            img_dir = os.path.join(self.dataset_root, 'instance_train')
            json_path = os.path.join(self.dataset_root, 'lane_line_class_7_train_v4.json')
        elif mode == 'val':
            img_dir = os.path.join(self.dataset_root, 'instance_val')
            json_path = os.path.join(self.dataset_root, 'lane_line_class_7_val_v4.json')
    ...
    def create_target(self, index):
        img_path = os.path.join(self.img_dir, self.images[index]["file_name"])
        img = cv2.imread(img_path, cv2.IMREAD_GRAYSCALE)
        # img2 = cv2.imread(img_path, cv2.IMREAD_COLOR)
        # data/qc_road_data/instance_train/lianxvbiandao_1338.pn
        # im = cv2.imread(img_path).astype('float32')

        # print(img.size())
        # print(img2.size())
        target = np.zeros(img.shape).astype(np.uint8)
        ...
        for anno in self.annos:
            if anno["image_id"] == image["id"]:
                ...
                for i in range(0, len(anno["segmentation"][0]), 2):
                    mask.append([anno["segmentation"][0][i], anno["segmentation"][0][i+1]])
                mask = np.array(mask, dtype=np.int32)
                cv2.fillPoly(target, [mask], anno["category_id"])
        # target = Image.fromarray(target, "L")
        return target
    
    def __getitem__(self, idx):

        image_path = os.path.join(self.img_dir, self.images[idx]["file_name"])
